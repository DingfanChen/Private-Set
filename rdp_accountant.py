...
def compute_epsilon(sigma, sampling_prob, iterations, delta, verbose=False):
    rdp = compute_rdp(sampling_prob, sigma, 1, DEFAULT_ALPHAS) * iterations
    eps, best_alpha = get_privacy_spent(DEFAULT_ALPHAS, rdp, delta)
    return float(eps)


def compute_sigma(epsilon, sampling_prob, iterations, delta, verbose=False):
    func = lambda x: compute_epsilon(x, sampling_prob, iterations, delta, verbose=verbose) - epsilon
    root = brentq(func, 1e-5, 20)
    return root
...
if __name__ == '__main__':
    ## Example
    SIGMA = 1.0  # noise
    SAMPLING_PROB = 256 / 60000  # Sampling prob. for SGD
    T = 1000 # Total iterations
    DELTA = 10 ** -5  # DP parameter; we fix delta and compute the minimum epsilon
    eps = 10
    print(eps)
    sigma = compute_sigma(eps, SAMPLING_PROB, T, DELTA)
    print('sigma', sigma)
    print(compute_epsilon(sigma, SAMPLING_PROB, T, DELTA))